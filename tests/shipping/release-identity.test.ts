...
function packed(version = "0.11.0"): unknown {
...
describe("release identity", () => {
  it("accepts matching immutable tag, manifest, lock root, and packed artifact versions", () => {
    expect(() => validateReleaseIdentity("v0.11.0", packageJson, lockfile, packed())).not.toThrow();
    expect(() =>
      validateReleaseIdentity("v0.11.0", packageJson, lockfile, {
        name: "local-llmup",
        version: "0.11.0",
        filename: "local-llmup-0.11.0.tgz",
      }),
    ).not.toThrow();
  });
  ...
  it("rejects a missing tag and every version mismatch", () => {
    ...
    expect(() => validateReleaseIdentity("v0.11.0", packageJson, lockfile, packed("0.6.0"))).toThrow(
      /version mismatch/u,
    );
    expect(() => validateReleaseIdentity("v0.6.0", packageJson, lockfile, packed("0.11.0"))).toThrow(
      /version mismatch/u,
    );
  });
});
