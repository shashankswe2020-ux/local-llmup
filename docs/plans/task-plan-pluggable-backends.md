...
- **Shared adapter contract suite** (B16): parameterized over every adapter —
  **explicit loopback bind (via a `--host` arg _or_ runtime env** so `ollama
...
#### B15: Catalog `gguf` sources + cited `(class, llamacpp)` perf rows _(blocked on D1/D2)_
...
llamacpp): **loopback bind is explicit** — via a `--host` arg _or_ runtime env,
...
#### B18: Apple-Silicon auto-detect priority + platform gating _(D3)_
