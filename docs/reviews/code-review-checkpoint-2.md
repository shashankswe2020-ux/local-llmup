...
- The "No active server to stop." status goes to stdout. It is consistent with the other `down` status lines (`Stopped`/`Detached`) also on stdout, so this is a judgment call; noting it only because these are status/diagnostic messages rather than result _data_. Fine to leave for consistency.
