...
- **Problem:** The Ollama branch always passes `expectedSizeBytes: quant.diskBytes`
  ...
  commit (`X-Repo-Commit` == `revision`) is *always* verified, so this is not
...
- **Problem:** `pull` unconditionally emits `downloading <file>` *before* calling
...
- The pull tests assert `events` *contains* `downloaded`/`cached` but never assert
...
- **One error-message change (edge case):** an Ollama model with *no* `ollama` source
...
- **Security reasoning is explicit and correct.** The doc comment on `chat` states
  precisely *why* `assertSafeModelId` is skipped (body-only, JSON-escaped, no argv/URL),
