...
- **Problem:** In `selectAttach`, `requested = flag ?? envBackend` short-circuits on the flag. If the flag matches `activeBackend` (valid) but `LOCAL_LLMUP_BACKEND` names a *different* backend, the env conflict is silently ignored rather than rejected. Spec §2.2 B says "a `--backend`/env value that conflicts with `active.backend` is a `ValidationError`" — treating each channel independently would honor that more literally.
- **Fix:** Validate both channels before resolving, e.g.:
  ```ts
  for (const [via, value] of [["--backend", flag], [ENV_BACKEND_OVERRIDE, envBackend]] as const) {
