...
- **Assessment:** Not in the T21 _acceptance_ list (which names backend/hw/catalog/state/digest), so folding disk into the summary is acceptable coverage — but a full disk is the single most common `pull` failure and is cheap to gate.
...
- **Correct probe-vs-display data handling.** `checkState` probes the _raw_ `active.endpoint` (right — you must dial the real value) while displaying only the `stripControl`'d copy. This is the subtle detail that is easy to get wrong.
