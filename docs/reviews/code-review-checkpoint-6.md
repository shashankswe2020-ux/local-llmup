...
- **Sanitization boundary is correct and load-bearing.** Every chat-sourced string (`user`, `assistant`, and therefore the extracted fact text and embedded chunk text) is `stripControl`'d before storage. This is the _right_ place: `JSON.stringify` only escapes the on-disk bytes; `JSON.parse` on the render path would restore the raw control/BiDi bytes, so `stripControl` — not JSON escaping — is what actually protects a later `chat`/`migrate` render. The "passes control-stripped content to the embedder" test locks this in.
