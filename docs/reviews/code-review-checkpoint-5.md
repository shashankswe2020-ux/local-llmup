...
- **No world-readable window on the meta file.** `writeFileSync(tempFile, json, { mode: FILE_MODE })` sets the _creation_ mode, which umask can only make _more_ restrictive — so the temp file is never broader than 0600 even before the belt-and-suspenders `chmodSync`. This correctly avoids the classic create-then-chmod race.
