...
- **Recommendation:** Enforce explicit caps at this boundary:
  - Reject or truncate `exchange.user` / `exchange.assistant` above a fixed byte budget (e.g. 64–256 KB) _before_ sanitizing/storing.
...
- **Fallible work runs before any disk write** — `prepareEmbedding` performs the network embed and validates vector count, per-vector dimension, and vector-space compatibility _before_ the first `appendFileSync`, so an embedder outage or mismatch aborts without persisting a half-recorded exchange.
